...
Two consumers of the same three TSVs, so interactive and static figures
never drift apart:

- **`bin/plot_accumulation_plotly.py`** — interactive HTML: accumulation
  curve (mean + permutation-band ribbon) for internal and external novelty,
  hoverable per-N values, and a per-clade contribution chart (sortable bar).
  For exploration and sharing with collaborators to poke at.
- **`bin/plot_accumulation_ggplot.R`** — static, publication-styled PDF: same
  two figures (curve + clade bars), fixed layout, for direct manuscript/
  report use.
